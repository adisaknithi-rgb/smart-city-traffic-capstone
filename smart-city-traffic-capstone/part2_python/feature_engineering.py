...
# ตั้งค่า Logger (ค่าเริ่มต้นเป็น INFO)
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

def create_features(df: pd.DataFrame) -> pd.DataFrame:
    # 1. Log Shape ก่อนทำ Feature Engineering (ตามข้อกำหนด)
    ...
    # Cyclical Encoding สำหรับ 24 ชั่วโมง
    ...
    # Derived Weather Indicator (ดัดแปลงอุณหภูมิเป็น Celsius และระบุสภาพอากาศรุนแรง)
    ...
    # --- [4. Data-driven Target Categorization] ---
    # คำนวณ Quartile Thresholds สำหรับจัดกลุ่ม Traffic Congestion
    q33 = df_feat['traffic_volume'].quantile(0.33)
    q66 = df_feat['traffic_volume'].quantile(0.66)
    
    # Log ค่า Thresholds ในระดับ DEBUG (จะไม่โชว์ในโหมดปกติ)
    logger.debug(f"Calculated Congestion Quantile Thresholds - 33rd: {q33:.2f}, 66th: {q66:.2f}")
    
    # สร้าง Categorical Target (Low, Medium, High)
    ...
    # ลบเฉพาะแถวที่เป็น NaN จากการทำ Lag/Rolling
    df_feat.dropna(subset=['traffic_lag_1h', 'traffic_rolling_mean_3h'], inplace=True)
    
    # Log Shape หลังทำ Feature Engineering (ตามข้อกำหนด)
